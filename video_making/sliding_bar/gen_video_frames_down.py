...
def diagonal_sliding_line(
    ...
    for pos in range(w + h, -h, -step):
        ...
        frame_path = os.path.join(output_folder, f"frame_{272-frame_idx:04d}.png")
        if 272 - frame_idx >= 71:
            cv2.imwrite(
                os.path.join(output_folder, f"frame_{272-frame_idx-71:04d}.png"),
                combined,
            )
        frame_idx += 1

    # # Save the final frame as well
    # final_frame_path = os.path.join(output_folder, f"frame_{272-frame_idx:04d}.png")
...
if __name__ == "__main__":
    ...
    text1 = "WildGS-SLAM\n      (Ours)"
    text2 = "Splat-SLAM"
    output_folder = "output/sliding_bar_frames_down"
